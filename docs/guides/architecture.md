...
4. 在有活动用户会话时检查 ClassIsland 进程数。只把可执行文件位于已配置安装根目录下一级 `app-*` 目录中的实例视为目标；同名的外部进程不会阻止恢复，也不会被终止。缺失时先观察两秒，给 ClassIsland 自主重启留出窗口；仍缺失时优先常规启动，常规启动失败时创建自动快照、尝试历史快照恢复，最后执行逃逸式启动。
5. 检测到多个可信目标进程时重启 ClassIsland；启动流程本身若发现重启重叠导致的多个实例，也会立即清理并重试一次，避免误触发快照恢复。每次启动前会尽力清除相应的 IFEO Debugger 项。
